...
const int moveInterval = 100;  // Adjust this for smooth motor updates
...
void loop() {
  ...
  if (currentTime - lastMoveTime >= moveInterval) {
      ...
      if (pointerElevation > 0) {
          Serial.print("Azimuth: "); Serial.println(pointerAzimuth);
          Serial.print("Elevation: "); Serial.println(pointerElevation);
...
void ElevationStepperGotoAngle(int angle)
{
  ...
  if (abs(stepsToMove) > 10)
  {
    ElevationStepper.setSpeed(15); // Faster speed for large movements
  }
  else
  {
    ElevationStepper.setSpeed(2); // Slower speed for small movements
  }
